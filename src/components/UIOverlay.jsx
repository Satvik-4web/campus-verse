  ...
  return (
    <motion.button
      ...
      className={`group relative flex h-[104px] w-full flex-row items-center gap-5 overflow-hidden
                  ...
                  sm:h-[380px] sm:flex-col sm:items-stretch sm:gap-0 sm:rounded-[2rem] sm:p-8
                  ${card.featured ? 'border-sky-400/30 bg-white/[0.04]' : 'border-white/10'} hover:border-[var(--accent)]/60`}
    >
      {/* Engineering dot grid, faded out toward the base */}
      <span
        className="pointer-events-none absolute inset-0 hidden opacity-[0.85] sm:block transition-opacity duration-700 group-hover:opacity-100"
        style={{
          backgroundImage: 'radial-gradient(circle at 1px 1px, color-mix(in srgb, var(--accent) 30%, transparent) 1px, transparent 0)',
      ...
      <span className={`pointer-events-none absolute inset-x-0 top-0 h-[2px] transition-all duration-700 ${card.featured ? 'bg-gradient-to-r from-transparent via-[var(--accent)] to-transparent opacity-80' : 'bg-gradient-to-r from-transparent via-white/20 to-transparent opacity-0 group-hover:opacity-100 group-hover:via-[var(--accent)]'}`} />
      ...
      {['left-4 top-4 border-l border-t', 'right-4 top-4 border-r border-t',
        ...
        <span
          key={pos}
          className={`pointer-events-none absolute hidden h-4 w-4 rounded-[3px] border-white/20 transition-all duration-700 group-hover:border-[var(--accent)]/80 group-hover:scale-110 sm:block ${pos}`}
        />
      ))}

      {/* Index + status */}
      <span className="relative z-10 hidden items-center justify-between sm:flex">
        <span className="font-mono text-[12px] font-medium tracking-[0.3em] text-white/40 transition-colors duration-700 group-hover:text-[var(--accent)]">
          {String(index + 1).padStart(2, '0')}
        </span>
        <span className="flex items-center gap-2 text-[9px] font-bold uppercase tracking-[0.25em] text-slate-400 group-hover:text-white transition-colors duration-700">
          <span className="h-1.5 w-1.5 rounded-full bg-[var(--accent)] shadow-[0_0_12px_var(--accent)] animate-pulse" />
          {card.status}
        </span>
      </span>
      ...
      <span className="relative z-10 flex min-w-0 flex-col">
        ...
        <span className="mt-1.5 text-[10px] font-medium tracking-[0.12em] text-slate-400 sm:mt-2 sm:text-[11px] group-hover:text-slate-300 transition-colors duration-700">
          {card.caption}
        </span>

        <span className="my-5 hidden h-px w-full bg-gradient-to-r from-white/20 via-white/5 to-transparent sm:block transition-all duration-700 group-hover:from-[var(--accent)]/40 group-hover:via-[var(--accent)]/10" />

        <span className="hidden items-center justify-between sm:flex">
          <span className="flex flex-col">
            <span className="truncate whitespace-nowrap text-[8px] font-bold uppercase tracking-[0.3em] text-slate-500 group-hover:text-slate-400 transition-colors duration-700">{card.meta}</span>
            <span className="mt-1.5 text-[11px] font-black uppercase tracking-[0.25em] text-[var(--accent)] group-hover:drop-shadow-[0_0_10px_var(--accent)] transition-all duration-700">
              {card.cta}
            </span>
          </span>
          <span className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-black/20 shadow-inner transition-all duration-700 group-hover:border-[var(--accent)]/50 group-hover:bg-[var(--accent)]/10 group-hover:shadow-[0_0_20px_color-mix(in_srgb,var(--accent)_30%,transparent)] group-hover:scale-110">
            <ArrowRight size={15} className="text-slate-400 transition-all duration-700 group-hover:translate-x-1 group-hover:text-white" />
          </span>
        </span>
      </span>
    </motion.button>
...
const UIOverlay = ({ menuState, activeView, setActiveView, smoothScroll }) => {
  const { isVRLoading, vrError, launchCampusTours, launchVRGame } = useKioskBridge();
  ...
  const onHubSelect = (id) => {
    ...
    else if (id === 'vr') setActiveView('vrgames');
  };
          ...
          <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="mb-auto grid w-full max-w-6xl grid-cols-1 items-start gap-8 landscape:grid-cols-[1fr_0.85fr] landscape:gap-12 mt-8 lg:mt-12">
                  ...
                  <div className="relative overflow-hidden rounded-[1.75rem] border border-white/[0.08] bg-gradient-to-b from-white/[0.05] to-transparent backdrop-blur-xl">
                    ...
                    <div className="relative h-[300px] w-full sm:h-[380px] lg:h-[440px] portrait:h-[400px]">
                      <KioskModel />
                    </div>
                ...
                <footer className="w-full mt-8 py-6 border-t border-white/[0.05] bg-slate-950/20 backdrop-blur-sm relative z-10">
            ...
            {activeView === 'vrgames' && (
              <motion.div
                ...
                className="absolute inset-0 flex flex-col items-center justify-center pointer-events-auto mt-16 sm:mt-0"
              >
                <div className="mb-10 flex flex-col items-center">
                  <span className="flex items-center gap-4">
                    <span className="h-px w-10 bg-gradient-to-r from-transparent to-cyan-400/50 sm:w-16" />
                    <span className="text-[9px] font-semibold uppercase tracking-[0.34em] text-cyan-300/80">
                      Immersive Arcade
                    </span>
                    <span className="h-px w-10 bg-gradient-to-l from-transparent to-cyan-400/50 sm:w-16" />
                  </span>
                  <h2 className="mt-3 text-3xl font-black tracking-[0.1em] text-white sm:text-4xl">VR GAMES</h2>
                  <span className="mt-2 text-[11px] font-light tracking-[0.06em] text-slate-400">
                    Select a game to launch directly to the headset
                  </span>
                </div>

                <div className="grid grid-cols-2 gap-6 sm:gap-10 landscape:lg:grid-cols-4">
                  {[
                    { id: 'metacrick', name: 'MetaCrick', sub: 'Ultimate', pkg: 'com.Capermint.VRCricket' },
                    { id: 'coverdrive', name: 'Cover Drive', sub: 'Cricket', pkg: 'io.mphillips.CoverDriveCricket22' },
                    { id: 'tennis', name: 'Tennis', sub: 'Esports', pkg: 'VRMotionLearning.TennisEsports' },
                    { id: 'badminton', name: 'Badminton', sub: 'Pro', pkg: 'com.ironstonegames.badmintonvr' },
                    { id: 'popone', name: 'Population', sub: 'ONE', pkg: 'com.BigBoxVR.PopulationONE' },
                    { id: 'roller', name: 'Epic Roller', sub: 'Coasters', pkg: 'games.b4t.epicrollercoasters.oculus' },
                    { id: 'vrfs', name: 'VRFS', sub: 'Football', pkg: 'com.immersification.vrfs' },
                    { id: 'roblox', name: 'Roblox', sub: 'VR', pkg: 'com.roblox.client' },
                  ].map((game) => (
                    <motion.div
                      key={game.id}
                      variants={{
                    ...
                    >
                      ...
                      <div className="relative flex h-20 w-20 items-center justify-center rounded-3xl bg-black/40 shadow-inner sm:h-28 sm:w-28 border border-white/10 transition-all duration-700 group-hover:scale-110 group-hover:border-cyan-400/60 group-hover:bg-cyan-950/30 group-hover:shadow-[0_0_30px_rgba(34,211,238,0.25)]">
                        <span className="text-4xl font-black text-white/50 transition-all duration-700 group-hover:text-cyan-300 group-hover:drop-shadow-[0_0_15px_rgba(34,211,238,0.8)] sm:text-5xl">{game.name.charAt(0)}</span>
                      </div>
                      
                      <div className="flex flex-col items-center text-center relative z-10">
                        <span className="text-lg font-black tracking-[0.12em] text-white sm:text-xl md:text-2xl group-hover:drop-shadow-[0_0_10px_rgba(255,255,255,0.5)] transition-all duration-700">{game.name}</span>
                        <span className="mt-2 text-[10px] font-bold uppercase tracking-[0.3em] text-cyan-500/70 group-hover:text-cyan-400 transition-colors duration-700 sm:text-[12px]">{game.sub}</span>
                      </div>
                    </motion.div>
                  ))}
                </div>
